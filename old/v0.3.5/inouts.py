...
def get_envase():
    with sqlite3.connect(db_path) as connection:
        ...
        envase_list = [{'id_envase': row[0], 'linha': row[1], 'fantasia_cliente': row[2],
                        'cod_item': row[3], 'desc_item': row[4], 'quantidade': row[5],
                        'data_entr_antec': row[6], 'data_envase': row[7],
                        'observacao': row[8], 'concluido': row[9]} for row in cursor.fetchall()]
    return envase_list
...
def producao():
    ...
    return render_template('pages/procesamento/processamento.html', producao=producao_list,
...
def calendar_producao():
    create_tables()
    producao_list = get_producao()
    return render_template('pages/procesamento/processamento-calendar.html', producao=producao_list)
...
def producao_edit():
    ...
    if request.method == 'POST':
        ...
        return render_template('pages/procesamento/processamento-edit.html', prod_edit=prod_edit,
                               user_permissions=user_permissions)
...
if __name__ == '__main__':
    ...
    if debug_dir in dir_os:
        ...
        sys.exit(2)
    print(f'''
    CDE | Versão: {app.config['APP_VERSION']} (beta) - Abr/2024
