...
def tlg_msg(msg):
    if not session.get('privilegio') == 1:
        if debug:
            print("A mensagem do Telegram não pôde ser enviada em modo debug")
        else:
...
def check_ip():
    ...
    if debug:
        current_server_ip = request.host_url
...
def get_frase():
    frases_motivacionais = [
        ...
        "O otimismo é a fé em ação. Tenha fé e alcance seus objetivos.",
        "A persistência é o segredo do sucesso. Continue tentando.",
...
def create_tables():
    with sqlite3.connect(db_path) as connection:
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS aux_permissions (
...
def get_saldo_for_export():
    with sqlite3.connect(db_path) as connection:
        ...
        cursor.execute('''
            ...
            ORDER BY desc_item
        ''')
        saldo_export = [{'produto': row[0], 'saldo': row[1], 'timestamp': row[2]} for row in cursor.fetchall()]
    return saldo_export
...
def get_saldo_view():
    with sqlite3.connect(db_path) as connection:
        ...
        saldo_visualization = [
            {'produto': row[0], 'saldo': row[1], 'ult_mov': row[2]} for row in cursor.fetchall()
        ]
...
def envase():
    create_tables()
    envase = get_envase()
    return render_template('pages/envase/envase.html', envase=envase)
...
def calendar_envase():
    create_tables()
    envase = get_envase()
    return render_template('pages/envase/envase-calendar.html', envase=envase)
...
def producao():
    ...
    return render_template('pages/procesamento/producao.html', producao=producao, user_permissions=user_permissions)
...
def calendar_producao():
    create_tables()
    producao = get_producao()
    return render_template('pages/procesamento/producao-calendar.html', producao=producao)
...
def producao_edit():
    ...
    if request.method == 'POST':
        ...
        return render_template('pages/procesamento/producao-edit.html', prod_edit=prod_edit,
...
def get_user_permissions(user_id):
    with sqlite3.connect(db_path) as connection:
        ...
        if rows:
            ...
            print('A pesquisa não encontrou nada.')
            return []
...
def get_permissions():
    with sqlite3.connect(db_path) as connection:
        cursor = connection.cursor()
        cursor.execute('''SELECT * FROM aux_permissions''')

        permissions = [{'id_perm': row[0], 'desc_perm': row[1]} for row in cursor.fetchall()]
    return permissions
...
def get_userdata(id_user):
    with sqlite3.connect(db_path) as connection:
        ...
        user_data = [{'privilege_user': row[0], 'nome_user': row[1], 'sobrenome_user': row[2],
                      'id_user': row[3], 'ult_acesso': row[4]} for row in cursor.fetchall()]
...
def users_edit():
    ...
    if request.method == 'POST':
        ...
        return render_template('pages/users/users-edit.html', user_permissions=user_permissions, permissions=permissions,
                               req_id_user=req_id_user, user_data=get_userdata(req_id_user))
...
def remove_permission(id_user, id_perm):
    with sqlite3.connect(db_path) as connection:
        ...
        cursor.execute('''DELETE FROM user_permissions 
                          WHERE id_user = ? AND id_perm = ?''', (id_user, id_perm))
        connection.commit()
...
def add_permission(id_user, id_perm):
    with sqlite3.connect(db_path) as connection:
        cursor = connection.cursor()
        cursor.execute('''INSERT INTO user_permissions (id_user, id_perm) 
                           VALUES (?, ?)''', (id_user, id_perm))
        connection.commit()
    ...
    if request.method == 'POST':
        ...
        try:
            with sqlite3.connect(db_path) as connection:
                ...
                cursor.execute('''SELECT id_user FROM users ORDER BY id_user DESC LIMIT 1''')
...
if __name__ == '__main__':
    ...
    if debug_dir in dir_os:
        #  Se o diretório contém debug_dir, modo = desenvolvedor.
        port = 5090
        debug = True
    elif 'CDEHP_Server$' in dir_os:
        #  Se o diretório contém 'CDEHP_Server$', modo = servidor.
        port = 5005
        debug = False
    else:
        ...
        input('''
        [ERRO 002] 
        O arquivo não pôde ser executado, verifique se o arquivo está alocado corretamente.
