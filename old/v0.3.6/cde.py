...
def producao():
    ...
    return render_template('pages/processamento/processamento.html', producao=producao_list,
...
def producao_edit():
    ...
    if request.method == 'POST':
        ...
        return render_template('pages/processamento/processamento-edit.html', prod_edit=prod_edit,
                               user_permissions=user_permissions, mode=mode)
...
if __name__ == '__main__':
    ...
    if dir_os in debug_dir:
        ...
        input(f'''
        [ERRO 002] {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}
        ...
        sys.exit(2)
    print(f'''
    Colaborative Developed Enviroment
    -+--+--+--+--+--+--+--+--+--+--+-
    Started in: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}
    Versão CDE: {app.config['APP_VERSION']} (beta) - Abr/2024
